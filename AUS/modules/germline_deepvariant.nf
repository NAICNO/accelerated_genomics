...
process DEEPVARIANT {
    ...
    // Nested by aligner (spec section 5h). Germline-only, so germline_mapping
    // always has a value -- no fallback needed.
    publishDir { "${params.outdir}/vcf/${params.germline_mapping}/deepvariant" }, mode: 'copy'
