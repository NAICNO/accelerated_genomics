...
#
# TDD state: expected RED until modules/giraffe.nf and the germline_workflow.nf
# / germline.config / publishDir changes exist (spec sections 11-13, 5h).
...
must_have() {  # file, fixed-string pattern, description
  if [ -f "$1" ] && grep -qF -- "$2" "$1"; then pass "$3"; else miss "$3"; fi
}
must_not_have() {
  if [ -f "$1" ] && ! grep -qE -- "$2" "$1"; then pass "$3"; else miss "$3"; fi
}
...
for p in graph_gbz graph_dist graph_min graph_zipcodes graph_ref_paths; do
  if [ -f "$CFG" ] && grep -qE "^\s*${p}\s*=\s*null" "$CFG"; then pass "$p = null"; else miss "$p = null"; fi
done
