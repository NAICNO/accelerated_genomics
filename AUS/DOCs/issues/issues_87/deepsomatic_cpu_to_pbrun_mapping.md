...
| `--reads_normal` | normal BAM | `--in-normal-bam IN_NORMAL_BAM` | **Required** - **Tumor-only mode may not be supported at all (Need to verify) ** |
...
| `--use_default_pon_filtering=false` | tumor-only PON filtering toggle | **No equivalent flag found.** | Flag only makes sense in Google's tumor-only mode  |
